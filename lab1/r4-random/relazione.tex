...
Eseguiamo 400 lanci di sei dadi distinti\footnote{Li distinguiamo in base al colore: rosso, verde, blu, viola, nero, bianco.},
...
in quanto i risultati dei dadi sono indipendenti fra loro e i valori possibili di $x_s$
sono in numero finito. Di conseguenza, la distribuzione di probabilità di $x_s$ è data da:
\[
    p \left(x_s=k\right) =
        \binom{6}{k}
...
di $x_i$, la distribuzione di probabilità di $x_i$ è data da una Poissoniana\footnote{
    $x_i$ può infatti assumere infiniti valori ($\left|\mathbb{N}\right|=\aleph_0$).
}:
\[
...
è\footnote{
    Il $\Th$, in realtà, decade $\alpha$:
    \[\Th \longrightarrow\; ^{228}_{\;\;88}\text{Ra}^* +\; ^4_2\alpha\]
    ma il radio si trova in uno stato eccitato, per cui emette in media un $\gamma$:
    \[^{228}_{\;\;88}\text{Ra}^* \longrightarrow\; ^{228}_{\;\;88}\text{Ra} + \gamma\]
    Il nostro campione si trovava all'interno di una busta di plastica, che scherma gli $\alpha$,
    ma permette il passaggio di $\gamma$: ecco perché ci è possibile associare
    i decadimenti del $\Th$ ai conteggi di $\gamma$ rilevati dal contatore.
}, nel caso del $\Th$, $\overline{X} = \tau^{-1}N = {T^{-1}_\frac{1}{2}}N\ln{2}$,
...
\[\diam = \left(14.00\pm0.05\right)\unit{mm}\]
...
risultanti come accennato alla nota 10.
}:
\[\begin{aligned}
...
è giustificata dalla seguente proposizione (che segue dalla legge dei grandi numeri):
...
se il dado è “truccato” (nel senso comune, informale, del termine), non è detto che la nostra stima lo valuti come tale: infatti, ogni faccia
...
(sei dadi a $N = 6$ facce, $n = 400$ lanci ciascuno).
...
onesti (verde, blu, bianco), due dadi truccati (rosso, viola)
