...
\begin{abstract}
    ...
    $m_{met.} c_{met.} (T_{caldo}-T_{eq.}=c_{acqua} m_{acqua} (T_{eq.}-T{ambiente})$
    "Il calorimetro è abbastanza adiabatico e questo lo valuterete nell'ultima parte 
    dell'esperimento."

    $m_{met.} c_{met.} (T_{caldo}-T_{eq.}=c_{acqua} m_{acqua} (T_{eq.}-T{ambiente})+
    C_{cal.} (T_{eq.}-T{ambiente})$. Questa è la seconda parte dell'esperimento.

    È necessaria una prima parte dell'esperimento per misurare ...
    ...
    $m_{calda} c_{calda} (T_{calda}-T_{eq.}=c_{fredda} m_{fredda} (T_{eq.}-T{ambiente})+
    C_{cal.} (T_{eq.}-T{ambiente})$
    
    Invece che misurare $C_{cal.}$ in $J/K$, possiamo considerare a quanta acqua equivale
    il calorimetro dal punto di vista termico, ovvero la quantità di acqua che assorbe lo 
    stesso calore del calorimetro.

    $m_{calda} c_{acqua} (T_{calda}-T_{eq.}=c_{acqua} m_{fredda} (T_{eq.}-T{ambiente})+
    m_{eq.} c_{acqua} (T_{eq.}-T{ambiente})$. La massa equivalente ci da anche un idea di
    ...
    di tempo che impiega l'acqua all'interno del calorimetro ad abbassare la sua
    temperatura di $e(T_{0}-T_{amb.})$ volte.

    La legge che segue questa discesa è: $(T-T_{amb.})=(T_{0}-T_{amb.}) e^{-t/\Tau}$.

    Il parametro $\Tau$ descrive quanto bene il calorimetro trattenga il calore
    (quindi sia adiabatico?). Per misurarlo abbiamo scaldato 200g d'acqua, che abbiamo
    ...
    $ln(T-T_{amb.})=ln(T_{0}-T_{amb.})-(1/\Tau)t$.
