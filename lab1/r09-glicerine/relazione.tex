...
\begin{enumerate}
  ...
  \item Che $\overline{d}_k$ non superi, in ordine di grandezza,
    $10^{-3}\,\unit{m}$.
\end{enumerate}
...
$\vec{F}_\eta = -6\pi\eta \overline{r}_k v_i\hat{z}$.

Allora, dalla prima legge di Newton:
\[
  \frac{4}{3}\pi g\overline{r}_k^3 (\rho_\text{sf} - \rho_\text{sol}) \hat{z}
  -6\pi\eta \overline{r}_k v_i\hat{z} = 0
\]
ovvero:
\[
  v_i = \frac{2g(\rho_\text{sf} - \rho_\text{sol})}{9\eta} \overline{r}_k^2
\]

Si noti che entrambe le forze hanno direzione parallela a $\hat{z}$:
ciò è coerente con l'ipotesi (1.).
